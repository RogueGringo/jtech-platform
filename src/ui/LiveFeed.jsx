...
export default function LiveFeed({ config, terms }) {
  const feedSources = config.feedSources || [];

  const classify = (text) => classifyText(text, {
  ...
  const doRefresh = useCallback(async () => {
    ...
    try {
      const data = await fetchAllFeeds(feedSources, classify);
  ...
  return (
    <div style={{ padding: "32px", maxWidth: 1200 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <div style={{
            ...
            <div style={{ fontSize: 9, color: COLORS.textMuted }}>
              {liveCount}/{feedSources.length} sources
            </div>
      ...
      {loading && feedItems.length === 0 && (
        <div style={{ textAlign: "center", padding: 40, color: COLORS.textDim }}>
          ...
          <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 12 }}>
            Querying {feedSources.length} open-source feeds via CORS proxies
          </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ fontSize: 10, color: COLORS.textMuted, letterSpacing: 1 }}>
            DATA SOURCES — {liveCount}/{feedSources.length} ACTIVE
          </div>
          {lastRefresh && (
            <div style={{ fontSize: 9, color: COLORS.textMuted }}>
      ...
      <SourceVerifyLink sources={config.verifySources?.feed} />
    </div>
  );
}
